...
static void ENC28J60_SPI2_Init(void)
{
  ...
  RCC_APB2PeriphClockCmd(	RCC_APB2Periph_GPIOB|RCC_APB2Periph_GPIOC, ENABLE );//PORTB,D,G时钟使能 
 
  //这里PB12拉高,是为了防止NRF24L01和SPI FLASH影响.
	//因为他们共用一个SPI口.  
	GPIO_InitStructure.GPIO_Pin = GPIO_Pin_4|GPIO_Pin_6|GPIO_Pin_12;				 			// 端口配置
	GPIO_InitStructure.GPIO_Mode = GPIO_Mode_Out_PP; 		 		//推挽输出
 	GPIO_InitStructure.GPIO_Speed = GPIO_Speed_50MHz;		 		//IO口速度为50MHz
 	GPIO_Init(GPIOB, &GPIO_InitStructure);								  //根据设定参数初始化
 	GPIO_SetBits(GPIOB,GPIO_Pin_4|GPIO_Pin_6|GPIO_Pin_12);					 							//上拉
	
	GPIO_InitStructure.GPIO_Pin = GPIO_Pin_6|GPIO_Pin_7; 	//推挽  上拉			 
 	GPIO_Init(GPIOC, &GPIO_InitStructure);					 				//根据设定参数初始化
 	GPIO_SetBits(GPIOC,GPIO_Pin_6);						//上拉	
...
{		
	...
	if(ENC28J60_Read(MAADR5)== macaddr[0] && ENC28J60_Read(MAADR4)== macaddr[1] && ENC28J60_Read(MAADR3)== macaddr[2] && ENC28J60_Read(MAADR2)== macaddr[3] && ENC28J60_Read(MAADR1)== macaddr[4]&& ENC28J60_Read(MAADR0)== macaddr[5])return 0;//初始化成功
	else return 1; 	  
